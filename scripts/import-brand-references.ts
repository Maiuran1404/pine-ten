...
async function importBrandReferences() {
  ...
  for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
    ...
    for (const imageUrl of batchUrls) {
      try {
        ...
        console.log(
          `    ✓ Imported: "${classification.name}" (${classification.toneBucket}/${classification.energyBucket})`
        )
        totalSuccess++
  ...
  // Check final count
  const finalCount = await db.select({ id: brandReferences.id }).from(brandReferences)
