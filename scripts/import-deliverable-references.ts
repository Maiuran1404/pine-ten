...
function inferDeliverableType(name: string, description: string): string {
  ...
  if (text.includes('presentation') || text.includes('slide') || text.includes('pitch deck'))
  ...
  if (text.includes('meta ad') || text.includes('static ad') || text.includes('performance ad'))
    return 'static_ad'
  if (text.includes('instagram') || text.includes('insta') || text.includes('social media'))
...
function inferStyleAxis(name: string, description: string, colorSamples: string[]): string {
  ...
  if (text.includes('corporate') || text.includes('professional') || text.includes('business'))
    return 'corporate'
  if (text.includes('playful') || text.includes('fun') || text.includes('creative'))
    return 'playful'
  if (text.includes('premium') || text.includes('luxury') || text.includes('elegant'))
    return 'premium'
  ...
  for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
    ...
    for (const imageUrl of batchUrls) {
      try {
        ...
        console.log(`    ✓ Imported: "${classification.name}" (${deliverableType}/${styleAxis})`)
        totalSuccess++
