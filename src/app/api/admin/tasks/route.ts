...
import { db } from "@/db";
import { tasks, users, taskFiles, taskMessages, creditTransactions } from "@/db/schema";
...
export async function DELETE(request: NextRequest) {
  return withErrorHandling(async () => {
    ...
    logger.info({ taskIds: deleted.map(t => t.id), count: deleted.length }, "Tasks deleted by admin");
