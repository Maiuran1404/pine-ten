...
function getYouTubeThumbnailUrl(videoId: string, quality: "maxres" | "hq" | "mq" | "default" = "maxres"): string {
  const qualityMap = {
  ...
  return withErrorHandling(async () => {
    ...
    let videos = await db
      ...
      .orderBy(
        ...
        deliverableStyleReferences.createdAt
      );

    // Filter by tag if provided (done in JS since jsonb contains is complex)
    if (tag) {
      videos = videos.filter(v => 
        v.videoTags && Array.isArray(v.videoTags) && v.videoTags.includes(tag)
      );
    }
...
export async function DELETE(request: NextRequest) {
  return withErrorHandling(async () => {
    ...
    await db
      .delete(deliverableStyleReferences)
      .where(eq(deliverableStyleReferences.id, id));
