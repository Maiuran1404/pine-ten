...
export async function POST(request: NextRequest) {
  return withErrorHandling(
    async () => {
      ...
      // Get tests to run
      let testsToRun = await db.select().from(securityTests).where(eq(securityTests.isActive, true))
...
export async function DELETE(request: NextRequest) {
  return withErrorHandling(
    async () => {
      ...
      const [run] = await db.select().from(securityTestRuns).where(eq(securityTestRuns.id, id))
