...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    ...
    botToken: botToken ? `Set (${botToken.length} chars, starts with ${botToken.slice(0, 8)}...)` : "NOT SET",
    signingSecret: signingSecret ? `Set (${signingSecret.length} chars, starts with ${signingSecret.slice(0, 8)}...)` : "NOT SET",
    channels: {
      superadminAlerts: channels.superadminAlerts ? `Set (${channels.superadminAlerts})` : "NOT SET",
