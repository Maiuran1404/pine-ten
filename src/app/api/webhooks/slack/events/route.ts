...
async function handleEvent(event: NonNullable<SlackEventPayload['event']>) {
  ...
  switch (event.type) {
    ...
    case 'member_joined_channel':
      logger.info({ user: event.user, channel: event.channel }, 'Member joined channel')
      break

    case 'member_left_channel':
      logger.info({ user: event.user, channel: event.channel }, 'Member left channel')
      break

    case 'app_mention':
      // Future: Handle @mentions of the bot
      logger.info({ channel: event.channel, text: event.text?.slice(0, 100) }, 'App mentioned')
      break

    case 'reaction_added':
      // Future: Track engagement via reactions
      logger.info({ reaction: (event as { reaction?: string }).reaction }, 'Reaction added')
      break
