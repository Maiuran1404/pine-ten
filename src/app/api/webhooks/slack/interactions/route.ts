...
import { verifySlackSignature, updateMessage, openModal, notifySlack } from '@/lib/slack'
import {
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info({ type: payload.type, user: payload.user?.username }, 'Slack interaction received')
    ...
    return NextResponse.json({ error: 'Interaction handler failed' }, { status: 500 })
  }
}
...
async function handleFreelancerRejectModal(
  ...
  await openModal(payload.trigger_id, rejectFreelancerModal(userId, user.name))
}

async function handleFreelancerRejectSubmit(payload: SlackInteractionPayload): Promise<void> {
  ...
  const reason = payload.view.state?.values?.rejection_reason?.reason_input?.value || ''
...
async function handleTaskVerify(taskId: string, payload: SlackInteractionPayload): Promise<void> {
  // Get task
...
async function handleTaskChangesModal(
  ...
  // Open changes modal
  await openModal(payload.trigger_id, requestTaskChangesModal(taskId, task.title))
}

async function handleTaskChangesSubmit(payload: SlackInteractionPayload): Promise<void> {
  if (!payload.view) return
  ...
  // Get feedback
  const feedback = payload.view.state?.values?.change_feedback?.feedback_input?.value || ''
  ...
  const [client, freelancer] = await Promise.all([
    ...
    task.freelancerId ? db.query.users.findFirst({ where: eq(users.id, task.freelancerId) }) : null,
  ])
  ...
  logger.info({ taskId, requestedBy: payload.user.name }, 'Task changes requested via Slack')
}
