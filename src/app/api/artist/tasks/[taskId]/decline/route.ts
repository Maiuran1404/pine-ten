...
export async function POST(
  ...
  return withErrorHandling(async () => {
    ...
    const result = await withTransaction(async (tx) => {
      ...
      const maxOffersPerLevel = currentEscalation === 1
      ...
      if (availableArtists.length > 0) {
        ...
        const minutes = algorithmConfig.acceptanceWindows[
          (task.urgency?.toLowerCase() || "standard") as keyof typeof algorithmConfig.acceptanceWindows
        ];
