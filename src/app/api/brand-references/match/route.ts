    ...
    const references = await db
      .select({
        ...
        score: sql<number>`
          ...
          (CASE WHEN ${
            visualStyle ? sql`${visualStyle} = ANY(${brandReferences.visualStyles})` : sql`false`
          } THEN 1 ELSE 0 END)
        `.as('score'),
      })
      .from(brandReferences)
      .where(eq(brandReferences.isActive, true))
      .orderBy(desc(sql`score`), brandReferences.displayOrder, desc(brandReferences.usageCount))
      .limit(limit)
      .offset(offset)
