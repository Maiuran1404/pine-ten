...
export async function GET(request: NextRequest) {
  return withErrorHandling(async () => {
    ...
    if (briefId) {
      ...
      return successResponse({ brief: convertDbBriefToLiveBrief(brief) })
    }
    ...
    if (draftId) {
      ...
      if (!isValidUUID(draftId)) {
        ...
        return successResponse({ brief: null })
      }

      const brief = await db.query.briefs.findFirst({
        where: and(eq(briefs.draftId, draftId), eq(briefs.userId, session.user.id)),
      })
...
export async function POST(request: NextRequest) {
  return withErrorHandling(async () => {
    ...
    if (!liveBrief) {
      throw Errors.badRequest('Missing brief data')
    }
...
export async function DELETE(request: NextRequest) {
  return withErrorHandling(async () => {
    ...
    if (!briefId) {
      throw Errors.badRequest('Brief ID required')
    }

    await db.delete(briefs).where(and(eq(briefs.id, briefId), eq(briefs.userId, session.user.id)))
