...
export async function GET(_request: NextRequest) {
  return withErrorHandling(
    ...
    { endpoint: 'GET /api/freelancer/payouts' }
  )
}
...
export async function POST(request: NextRequest) {
  return withErrorHandling(
    async () => {
      ...
      const connectAccount = await getConnectAccount(userId)
      if (!connectAccount) {
        throw Errors.badRequest('Please connect your Stripe account first')
      }

      if (!connectAccount.payoutsEnabled) {
        throw Errors.badRequest('Please complete Stripe onboarding to enable payouts')
      }
      ...
      if (creditsAmount > actualAvailable) {
        throw Errors.badRequest(`Insufficient balance. Available: ${actualAvailable} credits`)
      }
      ...
      if (!transferResult.success) {
        throw Errors.internal(transferResult.error || 'Payout processing failed')
      }
    ...
    { endpoint: 'POST /api/freelancer/payouts' }
  )
}
