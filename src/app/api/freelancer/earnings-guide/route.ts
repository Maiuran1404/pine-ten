...
export async function GET() {
  try {
    ...
    const earningsGuide = categories.map((category) => {
      ...
      const simpleEarnings = Math.round(baseCredits * complexityMultipliers.simple * artistEarningsPerCredit);
      const moderateEarnings = Math.round(baseCredits * complexityMultipliers.moderate * artistEarningsPerCredit);
      const complexEarnings = Math.round(baseCredits * complexityMultipliers.complex * artistEarningsPerCredit);
      const premiumEarnings = Math.round(baseCredits * complexityMultipliers.premium * artistEarningsPerCredit);
...
function getExamplesForCategory(
  ...
  const examplesByCategory: Record<string, Array<{ description: string; earning: number; complexity: string }>> = {
    "video-motion-graphics": [
      { description: "Simple logo animation (5-10 seconds)", earning: earnings.simple, complexity: "Simple" },
      { description: "Social media video ad (15-30 seconds)", earning: earnings.moderate, complexity: "Moderate" },
      { description: "Product showcase video with effects", earning: earnings.complex, complexity: "Complex" },
      { description: "Full campaign video with motion graphics", earning: earnings.premium, complexity: "Premium" },
    ],
    "static-ads": [
      { description: "Single static banner ad", earning: earnings.simple, complexity: "Simple" },
      { description: "Multi-size ad set (3-5 sizes)", earning: earnings.moderate, complexity: "Moderate" },
      { description: "Campaign ad set with variations", earning: earnings.complex, complexity: "Complex" },
      { description: "Full brand campaign (10+ assets)", earning: earnings.premium, complexity: "Premium" },
    ],
    "social-media-content": [
      { description: "Single social media post", earning: earnings.simple, complexity: "Simple" },
      { description: "Post series (3-5 posts)", earning: earnings.moderate, complexity: "Moderate" },
      { description: "Multi-platform content pack", earning: earnings.complex, complexity: "Complex" },
    ...
    "ui-ux-design": [
      ...
      { description: "Multi-page website design", earning: earnings.complex, complexity: "Complex" },
      { description: "Full app UI/UX with prototypes", earning: earnings.premium, complexity: "Premium" },
    ],
  };
