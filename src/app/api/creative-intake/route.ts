...
import type { ServiceType, IntakeMessage, GenericIntakeData } from '@/lib/creative-intake/types'
...
export async function POST(req: NextRequest) {
  return withErrorHandling(async () => {
    ...
    const { serviceType, currentStep, messages, userMessage, currentData } =
      creativeIntakeSchema.parse(body) as IntakeRequest
...
`
    ...
    const extractedData = extractDataFromMessage(serviceType, currentStep, userMessage, currentData)
    ...
    case 'social_ads': {
      ...
      if (currentStep === 'product_goal') {
        ...
        const cleanedMessage = message
          .replace(
            /(?:drive|get|generate|increase|build)\s+(?:sales|signups|awareness|leads)/gi,
            ''
          )
