...
const CardHeader = ({ icon: Icon, title, description }: { icon: React.ElementType; title: string; description: string }) => (
  ...
  const formatTransactionAmount = (amount: number, type: string) => {
    ...
    return (
      <span className={isPositive ? "text-emerald-500" : "text-rose-500"}>
        {isPositive ? "+" : "-"}{Math.abs(amount)} credits
      </span>
    );
  };
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-baseline gap-2">
                    <span className={cn(
                      "text-4xl font-bold",
                      billingData?.credits === 0 ? "text-rose-500" :
                      (billingData?.credits ?? 0) <= 2 ? "text-amber-500" : "text-emerald-500"
                    )}>
