  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className="absolute top-0 left-1/2 -translate-x-1/2 w-[60%] h-16 rounded-full opacity-20 blur-[60px]"
          style={{
        ...
        <div className="relative p-5 sm:p-6">
          ...
          <div className="flex items-start gap-4 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 group/name">
                {editingSection === 'name' ? (
                  <input
                    ...
                    className="text-2xl sm:text-3xl text-white font-bold bg-transparent border-b border-white/20 focus:border-crafted-olive outline-none w-full"
                    style={{ fontFamily }}
                  ...
                  <h1
                    className="text-2xl sm:text-3xl text-white font-bold truncate"
                    style={{ fontFamily }}
                  >
              ...
              {/* Industry tag + social links inline */}
              <div className="flex items-center gap-2 mt-1">
                {brandData.industry && (
                  <span className="inline-block px-2 py-0.5 rounded-full text-[10px] text-white/50 bg-white/5 border border-white/8">
                    {brandData.industry}
                  </span>
                )}
          ...
          {isDescriptionLong && (
            <button
              ...
              className="text-crafted-olive text-[10px] font-medium mt-0.5 hover:text-white transition-colors"
            >
      ...
      {/* === Sections 2-4: Two-column grid on desktop === */}
      <motion.div variants={staggerItem} className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
        {/* Left column: Visual Language */}
        <div
        ...
        >
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: `0 3px 10px ${getColorValue(which)}30`,
                    }}
                  />
                  <span className="text-white/40 text-[9px] capitalize">{which}</span>
                </button>
              ))}
            </div>
          ...
          >
            <span
              className="text-xl text-white/80 font-semibold leading-none"
              style={{ fontFamily }}
            >
              Aa
            </span>
            <div>
              <span className="text-white text-xs font-medium" style={{ fontFamily }}>
                {brandData.primaryFont || 'Satoshi'}
              </span>
              <span className="text-white/30 text-[10px] block">Primary typeface</span>
            </div>
          </button>
        ...
        {/* Right column: Personality + Audiences stacked */}
        <div className="flex flex-col gap-4">
          ...
          >
            <div className="flex items-center gap-2 mb-2.5">
              <Paintbrush className="w-3.5 h-3.5 text-white/40" />
              <span className="text-white/40 text-[10px] font-medium uppercase tracking-wider">
                Brand Personality
              </span>
            </div>
            ...
            <div className="space-y-2">
              ...
              <button
                ...
                className={`px-3 py-2 rounded-lg text-left transition-all w-full ${
                  editingSection === 'style'
                    ? 'bg-white/10 ring-1 ring-white/20'
                    : 'bg-white/[0.03] hover:bg-white/5 border border-white/8'
                }`}
              >
                <span className="text-white/40 text-[9px] uppercase tracking-wider block mb-0.5">
                  Visual Style
                </span>
                <span className="text-white text-xs font-medium">{visualStyleLabel}</span>
              </button>
              ...
              >
                ...
                <span className="text-white text-xs font-medium">{brandToneLabel}</span>
              </button>
            </div>
          ...
          {/* Target Audiences */}
          <div
            className="rounded-xl p-4 sm:p-5 flex-1"
            style={{
              background: 'var(--surface-overlay)',
              border: '1px solid var(--border-subtle)',
            }}
          >
            <div className="flex items-center justify-between mb-2">
              ...
              {!showAddAudience && (
                <button
                  ...
                  className="text-[10px] text-crafted-olive hover:text-white transition-colors font-medium"
                >
                  + Add
                </button>
              )}
                  ...
                  return suggestions
                    ...
                    .map((suggestion) => (
                      <button
                        ...
                        className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-white/5 text-white/60 hover:bg-crafted-olive/20 hover:text-crafted-olive border border-white/8 transition-colors"
                      >
                        + {suggestion}
                      </button>
                    ))
              ...
              >
                <div className="flex gap-1.5 mb-1.5">
                  <button
                    ...
                    className={`flex-1 py-1 rounded-md text-[9px] font-medium transition-colors ${
                      newAudienceType === 'b2b'
                ...
                <div className="flex gap-1.5">
                  <input
                    ...
                    placeholder={
                      newAudienceType === 'b2b' ? 'e.g., HR Leaders' : 'e.g., Job seekers'
                    }
                    className="flex-1 px-2 py-1 rounded-md text-[10px] text-white bg-white/5 border border-white/10 focus:border-crafted-olive outline-none"
                    onKeyDown={(e) => e.key === 'Enter' && handleAddAudience()}
                  />
                  <button
                    onClick={handleAddAudience}
                    className="px-2 py-1 rounded-md text-[9px] font-medium bg-crafted-olive text-black/80 hover:bg-white transition-colors"
                  >
                    Add
                  </button>
                  <button
                    ...
                    className="px-1.5 py-1 rounded-md text-[9px] text-white/40 hover:text-white/70"
                  >
                    Cancel
                  </button>
                </div>
            ...
            {/* Existing audiences - compact rows */}
            {brandData.audiences && brandData.audiences.length > 0 ? (
              <div className="space-y-0.5">
                {brandData.audiences.map((audience, index) => (
                  <div
                    key={index}
                    className="px-2.5 py-1.5 rounded-lg flex items-center gap-2 group"
                    style={{
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      {editingAudienceIndex === index ? (
                        <input
                          ...
                          autoFocus
                          className="text-white text-xs font-medium bg-transparent border-b border-white/30 focus:border-crafted-olive outline-none w-full"
                    ...
                    {audience.isPrimary && (
                      <span
                        className="text-[8px] px-1 py-0.5 rounded-full font-medium shrink-0"
                        style={{
                    ...
                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                      <button
                        ...
                        className="p-0.5 rounded text-white/30 hover:text-white/70 hover:bg-white/5 transition-all"
                        title="Edit name"
                      >
                        <svg
                          className="w-3 h-3"
                      ...
                      {!audience.isPrimary && (
                        <button
                          ...
                          className="p-0.5 rounded text-white/30 hover:text-crafted-olive hover:bg-crafted-olive/10 transition-all"
                      ...
                      <button
                        ...
                        className="p-0.5 rounded text-white/30 hover:text-ds-error hover:bg-ds-error/10 transition-all"
                        title="Remove"
                      >
                        <svg
                          className="w-3 h-3"
              ...
              !showAddAudience && (
                <button
                  onClick={() => setShowAddAudience(true)}
                  className="w-full p-2.5 rounded-lg text-center text-white/40 text-xs hover:text-white/60 hover:bg-white/5 transition-colors"
