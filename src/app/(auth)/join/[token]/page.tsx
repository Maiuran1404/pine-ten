...
function JoinContent() {
  ...
  useEffect(() => {
    ...
    async function validateToken() {
      try {
        const response = await fetch(`/api/artist-invites/${token}`)
        const result = await response.json()

        if (!response.ok || !result.success) {
          setInviteData({ valid: false, reason: 'Unable to validate invite' })
          setValidating(false)
          return
        }
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      // Store invite token in sessionStorage for post-OAuth handling
      sessionStorage.setItem('artistInviteToken', token)

      await signIn.social({
        provider: 'google',
        callbackURL: `${window.location.origin}/join/${token}/callback`,
      })
    } catch (error) {
          ...
          >
            ...
            <div className="text-center">
              ...
              <p className="text-white/50 text-sm mb-6">
                {inviteData?.reason || 'This invite link is no longer valid.'}
              </p>
        ...
        >
          ...
          <div className="text-center mb-6">
            <h1
              ...
              style={{ fontFamily: "var(--font-satoshi, 'Satoshi'), sans-serif" }}
            >
              You&apos;ve been invited to join Crafted
            </h1>
            <p className="text-white/50 text-sm">Set up your account in 30 seconds</p>
          </div>
          ...
          >
            {isGoogleLoading ? <LoadingSpinner size="sm" /> : <GoogleIcon className="w-5 h-5" />}
