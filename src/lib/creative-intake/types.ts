...
export const SOCIAL_PLATFORMS = ["instagram", "linkedin", "facebook", "tiktok", "snapchat", "twitter"] as const;
...
// Video-specific
export const VIDEO_TYPES = ["ugc", "talking_head", "screen_recording", "event", "podcast_clip"] as const;
...
export const CONTENT_TYPES_SOCIAL = ["educational", "behind_the_scenes", "product", "memes", "storytelling"] as const;
...
export const LAUNCH_ASSETS = ["logo", "product_photos", "ui_screenshots", "figma", "website"] as const;
...
export interface BrandPackageIntake {
  ...
  includesItems: ("logo" | "social_templates" | "brand_guidelines" | "business_cards" | "presentations")[];
  // Context
...
export const INFERENCE_PATTERNS: Record<ServiceType, InferenceRule[]> = {
  launch_video: [
    { field: "platforms", triggers: ["students", "gen z", "young"], value: "tiktok,reels", confidence: 0.8 },
    { field: "platforms", triggers: ["b2b", "professional", "enterprise"], value: "linkedin,youtube", confidence: 0.8 },
  ...
  video_edit: [
    ...
    { field: "music", triggers: ["energetic", "fun", "trending"], value: "trendy", confidence: 0.7 },
  ],
  pitch_deck: [],
  brand_package: [
    { field: "includesItems", triggers: ["startup", "new company"], value: "logo,brand_guidelines", confidence: 0.8 },
  ],
  social_ads: [
  ...
  social_content: [
    { field: "frequency", triggers: ["aggressive", "growth", "fast"], value: "5x_week", confidence: 0.7 },
    { field: "contentTypes", triggers: ["thought leader", "expert"], value: "educational,storytelling", confidence: 0.8 },
  ],
};
...
export function getRecommendedVideoLength(platforms: VideoPlatform[]): string {
  ...
  const lengths = platforms.map((p) => SMART_DEFAULTS.videoLength[p]);
