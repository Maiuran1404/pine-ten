...
import { createEmptyIntakeState, SERVICE_DEFINITIONS } from '@/lib/creative-intake/types'
import {
  getFlowConfig,
  getFlowStep,
  getNextStep,
...
export function useCreativeIntake(options: UseCreativeIntakeOptions = {}): UseCreativeIntakeReturn {
  ...
  // Add message helper
  const addMessage = useCallback((message: Omit<IntakeMessage, 'id' | 'timestamp'>) => {
  ...
  // Update data helper
  const updateData = useCallback((updates: GenericIntakeData) => {
    setState((prev) => {
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      try {
        ...
        const nextStepId =
          result.nextStep ||
          getNextStep(state.serviceType, currentStepId, { ...state.data, ...result.extractedData })
        ...
        if (nextStepId) {
          ...
          addMessage({
            ...
            content: parsed.text,
            questionType:
              nextStep?.questionType === 'confirmation'
                ? 'confirmation'
            ...
            quickOptions:
              parsed.quickOptions?.options ||
              nextStep?.quickOptions?.map((opt) => ({
                label: opt.label,
                value: opt.value,
                variant: opt.variant,
              })),
  ...
  const submitGroupedAnswers = useCallback(
    (answers: Record<string, string | string[]>) => {
      ...
      // Determine next step
      const nextStepId = getNextStep(state.serviceType, currentStepId, {
        ...state.data,
        ...answers,
      })
  ...
  // Calculate progress
  const progress =
    state.serviceType && currentStepId ? calculateFlowProgress(state.serviceType, currentStepId) : 0
  ...
  switch (serviceType) {
    case 'launch_video':
      ...
      if (data.platforms) {
        items.push({
          label: 'Platforms',
          value: data.platforms as string[],
          source: 'user',
          editable: false,
        })
      }
      ...
      if (data.storylinePreference) {
        items.push({
          label: 'Storyline',
          value:
            data.storylinePreference === 'create_for_me' ? "We'll create one" : 'You have ideas',
          source: 'user',
          editable: false,
        })
      }
    ...
    case 'brand_package':
      ...
      if (data.includesItems) {
        items.push({
          label: 'Includes',
          value: data.includesItems as string[],
          source: 'user',
          editable: false,
        })
      }
    ...
    case 'social_content':
      ...
      if (data.recommendedContentTypes) {
        items.push({
          label: 'Content Types',
          value: data.recommendedContentTypes as string[],
          source: 'default',
          editable: false,
        })
      }
...
function generateRecommendations(serviceType: ServiceType, data: GenericIntakeData): string[] {
