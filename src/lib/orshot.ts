...
export async function generateDesignFromTemplate(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.error({ status: response.status, error: errorData, templateId }, 'Orshot API error')
    ...
    logger.info({ templateId, responseTime, format }, 'Design generated successfully')
    ...
    logger.error({ err: error, templateId, responseTime }, 'Failed to generate design from Orshot')
