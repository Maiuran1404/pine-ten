...
export async function getUserStylePreferences(
  ...
  typeMap.forEach((axes, type) => {
    ...
    const scoredAxes = axes.map(a => ({
      ...a,
      preferenceScore: Math.round((a.count / typeMax) * 70 + (a.confirmedCount / Math.max(a.count, 1)) * 30),
    }));
...
export async function getHistoryBoostScores(
  ...
  const typePrefs = preferences.byDeliverableType.find(
    p => p.deliverableType === deliverableType
  );
