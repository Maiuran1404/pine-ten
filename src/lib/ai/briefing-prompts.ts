...
- Each scene MUST include ALL of these fields:
  ...
  - imageGenerationPrompt: 2-3 sentence visual description for AI image generation. Priority order: (1) NAME THE SPECIFIC SUBJECT of this scene — the actual product, person, device, or interaction. Be concrete: "smartphone showing failed identity scan with red error overlay" not "elegant product arrangement". (2) Describe the SETTING and ACTION — where it happens and what's happening. (3) Apply the selected visual style as a MODIFIER to the subject — lighting, color grading, and mood should enhance the subject, not replace it. CRITICAL: The subject of the scene must ALWAYS match the storyline. Never write generic descriptions like "luxury still-life", "elegant product on surface", or "premium arrangement" — always depict the specific narrative moment.
