...
const TASK_TYPE_PATTERNS: PatternMatch[] = [
  ...
  {
    pattern: /\b(\d+)\s*(day|week|month)\s*(content\s*)?(plan|calendar|schedule)\b/i,
    value: 'multi_asset_plan',
    confidence: 0.95,
  },
  {
    pattern: /\b(content\s*)?(plan|calendar|schedule)\s*for\s*(\d+)\s*(day|week|month)/i,
    value: 'multi_asset_plan',
    confidence: 0.95,
  },
  {
  ...
  {
    pattern: /\b(create|make|design)\s*(a|an|the)?\s*(post|image|graphic|thumbnail)\b/i,
    value: 'single_asset',
    confidence: 0.7,
  },
  // 2 items is borderline - could be single design with variations
  {
    pattern: /\b(2|two)\s*(post|posts|image|images|graphic|graphics|carousel|carousels)\b/i,
    value: 'single_asset',
    confidence: 0.6,
  },
]
...
function extractAudienceFromText(text: string): AudienceBrief | null {
  ...
  for (const pattern of audiencePatterns) {
    ...
    if (match && match[1]) {
      ...
      if (audienceText.length > 3 && audienceText.length < 100) {
        ...
        const demographics = ageMatch ? `Ages ${ageMatch[1]}-${ageMatch[2]}` : undefined
  ...
  const demographicKeywords = [
    ...
    {
      pattern: /\b(health\s*conscious|fitness\s*enthusiasts?|health\s*focused)\b/i,
...
function formatDemographics(demographics?: InferredAudience['demographics']): string | undefined {
...
export function generateClarifyingQuestions(
  ...
  // Intent question
  if (!inference.intent.value || inference.intent.confidence < CONFIDENCE_THRESHOLD) {
...
export function applyInferenceToBrief(
  ...
  if (brandAudiences && brandAudiences.length > 0) {
    ...
    if (
      audienceMatch.value &&
      (audienceMatch.confidence > (updated.audience.confidence || 0) || !updated.audience.value)
    ) {
  ...
  if (hasAnyContent || messageText) {
    ...
    const avgConfidence =
      confidences.length > 0 ? confidences.reduce((a, b) => a + b, 0) / confidences.length : 0.3
    ...
    if (shouldUpdateSummary) {
      ...
      const isSummaryValid = (s: string) => {
        ...
        if (/^(of|for|to|in|on|at|by|with|my|our|the|a|an)\b/i.test(s) && s.split(' ').length <= 2)
          return false
        return true
      }
  ...
  if (!updated.taskSummary.value && messageText) {
    ...
    if (
      fallbackSummary &&
      fallbackSummary !== 'New project request' &&
      fallbackSummary.length >= 5
    ) {
      updated.taskSummary = {
