...
import { analyzeColorBucketFromHex, type ColorBucket, type StyleAxis } from "@/lib/constants/reference-libraries";
 ...
 * Common serif fonts
 */
const SERIF_FONTS = ["times", "georgia", "garamond", "playfair", "merriweather", "lora", "libre baskerville", "crimson", "pt serif", "noto serif"];

/**
 * Common sans-serif fonts
 */
const SANS_SERIF_FONTS = ["arial", "helvetica", "roboto", "open sans", "lato", "montserrat", "poppins", "inter", "nunito", "raleway", "source sans", "work sans", "dm sans", "satoshi", "geist"];

/**
 * Display/decorative fonts
 */
const DISPLAY_FONTS = ["impact", "bebas", "oswald", "archivo", "anton", "righteous", "bangers", "lobster"];

/**
 * Handwritten/script fonts
 */
const HANDWRITTEN_FONTS = ["brush", "script", "dancing", "pacifico", "sacramento", "satisfy", "caveat", "indie flower"];
...
function analyzeTypographyDNA(fonts: {
  ...
  const classifyFont = (font: string | null): "serif" | "sans-serif" | "display" | "handwritten" | "unknown" => {
 ...
 */
const INDUSTRY_STYLE_MAP: Record<string, {
  commonStyles: StyleAxis[];
  avoidStyles: StyleAxis[];
}> = {
  technology: { commonStyles: ["tech", "minimal", "corporate"], avoidStyles: ["organic", "playful"] },
  ...
  healthcare: { commonStyles: ["minimal", "corporate", "organic"], avoidStyles: ["bold", "playful"] },
  ...
  restaurant: { commonStyles: ["organic", "editorial", "playful"], avoidStyles: ["corporate", "tech"] },
  education: { commonStyles: ["playful", "minimal", "corporate"], avoidStyles: ["premium", "bold"] },
  entertainment: { commonStyles: ["bold", "playful", "editorial"], avoidStyles: ["corporate", "minimal"] },
  ...
  ecommerce: { commonStyles: ["minimal", "playful", "bold"], avoidStyles: ["corporate", "premium"] },
  media: { commonStyles: ["editorial", "bold", "minimal"], avoidStyles: ["corporate", "organic"] },
  consulting: { commonStyles: ["corporate", "minimal", "premium"], avoidStyles: ["playful", "bold"] },
  legal: { commonStyles: ["corporate", "minimal", "premium"], avoidStyles: ["playful", "bold", "organic"] },
  realestate: { commonStyles: ["premium", "corporate", "editorial"], avoidStyles: ["playful", "tech"] },
...
export async function extractStyleDNA(userId: string): Promise<StyleDNA | null> {
  ...
  const energyLevel = colorDNA.saturation === "vibrant" ? 70 :
    colorDNA.saturation === "muted" ? 30 : 50;

  const densityLevel = colorDNA.contrast === "high" ? 70 :
    colorDNA.contrast === "low" ? 30 : 50;

  const formalityLevel = typographyDNA.personality === "professional" ? 80 :
    typographyDNA.personality === "elegant" ? 70 :
    typographyDNA.personality === "playful" ? 20 :
    typographyDNA.personality === "modern" ? 50 : 50;

  const modernityLevel = typographyDNA.primaryStyle === "sans-serif" ? 70 :
    typographyDNA.primaryStyle === "serif" ? 30 :
    colorDNA.temperature === "cool" ? 60 : 50;
