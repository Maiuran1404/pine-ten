...
export async function scrapeBiggedUrls(
  ...
  try {
    ...
    while (
      allMedia.size < maxItems &&
      noChangeCount < MAX_NO_CHANGE_COUNT &&
      ...
      try {
        ...
        try {
          await iframe
            .locator('[class*="scroll"], [class*="content"]')
            .first()
      ...
      if (allMedia.size === prevCount) {
        ...
        try {
          const loadMoreBtn = iframe.locator(
...
async function checkDuplicates(
  ...
  const existingByUrl = await db
    .select({
      ...
      sourceUrl: deliverableStyleReferences.sourceUrl,
    })
    ...
    .where(
      or(
        ...
        ...sourceUrls
          .slice(0, 50)
          .map((url) => like(deliverableStyleReferences.imageUrl, `%${url.split('/').pop()}%`))
      )
    )
  ...
  for (const item of items) {
    ...
    const matchingName = Array.from(existingImageNames).find((name) => name?.includes(imageName))
    if (matchingName) {
