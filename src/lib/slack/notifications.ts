...
import { postMessage, getChannelConfig, isSlackConfigured, sendDM, lookupUserByEmail } from "./client";
  ...
  try {
    switch (event) {
      ...
      case "NEW_FREELANCER_SIGNUP":
        return notifyNewFreelancerSignup(
          data.user as SlackUserInfo,
          channels.newSignups
        );
      ...
      case "TASK_PENDING_REVIEW":
        return notifyTaskPendingReview(
          data.task as SlackTaskInfo,
          channels.pendingReviews
        );
