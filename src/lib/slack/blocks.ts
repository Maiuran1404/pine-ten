...
export function newClientSignupBlock(
  user: SlackUserInfo,
  ...
  return [
    ...
    actions([linkButton('View in Dashboard', `${APP_URL}/admin/users/${user.id}`)]),
  ]
}
...
export function newFreelancerSignupBlock(user: SlackUserInfo): (Block | KnownBlock)[] {
  return [
    ...
    section(`*${user.name}*\n` + `Email: ${user.email}`),
    context(`Signed up at ${formatDate(new Date())}`),
    actions([linkButton('View Profile', `${APP_URL}/admin/freelancers`)]),
  ]
}
...
export function taskCreatedBlock(task: SlackTaskInfo): (Block | KnownBlock)[] {
  return [
    ...
    actions([linkButton('View Task', `${APP_URL}/admin/tasks/${task.id}`)]),
  ]
}
...
export function taskVerifiedBlock(task: SlackTaskInfo, verifiedBy: string): (Block | KnownBlock)[] {
  return [
    section(
...
export function taskCancelledBlock(task: SlackTaskInfo, reason?: string): (Block | KnownBlock)[] {
  return [
    section(
...
export function creditPurchaseBlock(purchase: SlackCreditPurchaseInfo): (Block | KnownBlock)[] {
  return [
    section(
...
export function clientChannelWelcomeBlock(company: SlackCompanyInfo): (Block | KnownBlock)[] {
  return [
...
export function clientTaskUpdateBlock(
  ...
  return [
    ...
    context(formatDate(new Date())),
    actions([linkButton('View Details', `${APP_URL}/tasks/${task.id}`)]),
  ]
}
...
export function systemErrorBlock(error: string, context_info?: string): (Block | KnownBlock)[] {
  return [
    section(
...
export function securityAlertBlock(alertType: string, details: string): (Block | KnownBlock)[] {
  return [
    header('Security Alert'),
    divider(),
    section(`:shield: *${alertType}*\n` + details.slice(0, 1000)),
    context(formatDate(new Date())),
  ]
}
...
export function rejectFreelancerModal(freelancerId: string, freelancerName: string): object {
  return {
    type: 'modal',
...
export function requestTaskChangesModal(taskId: string, taskTitle: string): object {
  return {
    type: 'modal',
