...
export function generateSmartCompletion(text: string): string | null {
  ...
  const completions: Array<{ pattern: RegExp | string; completion: string }> = [
    { pattern: /^a\s+(tech\s+)?(product|app|platform|tool|software|service)\s*(\/\s*\w+)?\s+for\s*$/i, completion: "businesses to streamline their workflows" },
    { pattern: /^a\s+(saas|b2b|b2c)\s+(product|app|platform|tool)\s+for\s*$/i, completion: "managing team productivity" },
    ...
    { pattern: /^we\s+(have|built|created|made)\s+(a|an)\s*$/i, completion: "platform that helps businesses" },
    ...
    { pattern: /^my\s+(product|app|company|startup|business)\s*$/i, completion: "helps businesses save time" },
    { pattern: /^our\s+(product|app|company|startup|business)\s*$/i, completion: "is a platform for teams" },
    ...
    { pattern: /^(a|an)\s+(e-?commerce|shopping)\s*$/i, completion: "platform for small businesses" },
    ...
    { pattern: /that\s+helps\s+(people|users|businesses|teams|companies)\s*$/i, completion: "save time and money" },
    { pattern: /that\s+lets\s+(people|users|you)\s*$/i, completion: "manage their projects easily" },
    { pattern: /that\s+makes\s+(it\s+)?(easy|easier|simple)\s*$/i, completion: "to collaborate with teams" },
    { pattern: /that\s+automates\s*$/i, completion: "repetitive tasks" },
    { pattern: /that\s+connects\s*$/i, completion: "businesses with customers" },
    { pattern: /to\s+help\s+(people|users|businesses|teams)\s*$/i, completion: "be more productive" },
  ...
  const wordCompletions: Record<string, string> = {
    inst: "agram", link: "edIn", tikt: "ok", face: "book", yout: "ube", twit: "ter",
    mini: "mal", moder: "n", cine: "matic", prof: "essional", eleg: "ant",
    vibr: "ant", dynam: "ic", sleek: " and modern",
    prod: "uct", serv: "ice", plat: "form", busin: "esses", enter: "prise",
    start: "up", compan: "y",
    anno: "uncement", awar: "eness", laun: "ch", prom: "otion", conv: "ersion",
    sign: "ups", engag: "ement",
    stream: "line workflows", auto: "mate tasks", manag: "e their projects",
    track: "ing and analytics", colla: "borate",
  };
