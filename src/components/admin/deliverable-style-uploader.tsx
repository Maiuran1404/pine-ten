...
export function DeliverableStyleUploader({ onUploadComplete }: DeliverableStyleUploaderProps) {
  ...
  const classifyAll = async () => {
    ...
    for (let i = 0; i < pendingIds.length; i++) {
      ...
      try {
        ...
        const response = await fetch("/api/admin/deliverable-styles/upload", {
  ...
  const uploadAll = async () => {
    ...
    if (successCount > 0) {
      toast.success(`Successfully uploaded ${successCount} deliverable styles`);
      onUploadComplete?.();
    }
    if (errorCount > 0) {
      toast.error(`Failed to upload ${errorCount} deliverable styles`);
    }
  };
        ...
        >
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            {isDragActive ? (
              ...
              <>
                <p className="font-medium">Drag & drop style reference images here</p>
