...
export function useDeliverableStyleScraperData({ onUploadComplete }: UseDeliverableStyleScraperDataOptions = {}) {
  ...
  const updateClassification = useCallback((url: string, field: string, value: string | string[]) => {
    setClassifiedImages((prev) =>
      prev.map((img) =>
