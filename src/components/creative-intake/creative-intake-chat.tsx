...
import type { IntakeData, IntakeMessage } from '@/lib/creative-intake/types'
...
export function CreativeIntakeChat({ onComplete, onCancel, className }: CreativeIntakeChatProps) {
  const {
    state,
    isLoading,
    error,
  ...
  const serviceDefinition = state.serviceType ? SERVICE_DEFINITIONS[state.serviceType] : null
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-medium text-foreground">
                {state.serviceType ? serviceDefinition?.label : 'What do you need?'}
              </h2>
              {state.serviceType && (
        ...
        {state.stage === 'service_select' && (
          ...
          >
            <div className="text-center space-y-2 mb-6">
              <h3 className="text-lg font-medium">What can we help you create?</h3>
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                We&apos;ll start working on your {serviceDefinition?.label.toLowerCase()} right
                away.
              </p>
            </div>
          </motion.div>
...
function hasInteractiveElement(message?: IntakeMessage): boolean {
  ...
  return !!(message.groupedQuestions?.length || message.quickOptions?.length || message.summary)
}
