...
test.describe("Task Submission Flow", () => {
  ...
  test("should open credit dialog when insufficient credits", async ({
    page,
  }) => {
    // Navigate to chat
    await page.goto("/dashboard/chat");
  ...
  test("should preserve draft ID in return URL for Stripe", async ({
    page,
  }) => {
  ...
  test("should show submission modal after successful payment return", async ({
    page,
  }) => {
  ...
  test("should navigate to task page after successful submission", async ({
    page,
  }) => {
    // This test requires a user with sufficient credits
    // Skip if not in a proper test environment
    test.skip(
      !process.env.TEST_WITH_CREDITS,
      "Requires test user with credits"
    );
